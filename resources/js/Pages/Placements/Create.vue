...
const submit = () => {
    form.post(route('placements.store'));
};
</script>
        ...
        <template #header>
            <div class="flex items-center justify-between">
                ...
                <h2 class="flex items-center gap-2 text-xl font-semibold leading-tight text-slate-900">
                    <i class="fa-solid fa-plus text-sm text-slate-400"></i>
                    Create Placement
                </h2>
            </div>
        </template>
        ...
        <form @submit.prevent="submit" class="card card-body space-y-4">
                    <div class="grid gap-4 md:grid-cols-2">
                        <div>
                            ...
                            <select id="student" v-model="form.student_id" class="mt-2 block w-full rounded-xl border-slate-200 bg-white/90 px-3 py-2 text-sm text-slate-900 shadow-sm focus:border-emerald-400 focus:ring-emerald-400">
                    ...
                    <PrimaryButton :disabled="form.processing">
                        <i class="fa-regular fa-floppy-disk mr-2 text-sm"></i>
                        Save Placement
                    </PrimaryButton>
        </form>
    </AuthenticatedLayout>
